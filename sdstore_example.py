...
# it's saved to file in sd/sdstore/config.sdstore.py
...
# sd.get(name*, store="friends") = use friends.sdstore.py and sd.stores["friends"] instead of config.sdstore.py and sd.stores["config"].
...
# populates a whole storage file (name.sdstore.py) and dictionary (sd.stores[name]).
